...
def analyze_single_file(filepath: str, verbose: bool = False):
    ...
    # Overall stats
    print("\nOverall Statistics:")
    print(f"  Entries with execution details: {stats['with_exec_details']}/{stats['total']} ({stats['with_exec_details']/stats['total']*100:.1f}%)")
    print(f"  Compiled successfully: {stats['compiled']}/{stats['total']} ({stats['compiled']/stats['total']*100:.1f}%)")
    print(f"  Passed correctness: {stats['correct']}/{stats['total']} ({stats['correct']/stats['total']*100:.1f}%)")
    print(f"  Has performance data: {stats['with_speedup']}/{stats['total']} ({stats['with_speedup']/stats['total']*100:.1f}%)")
...
def print_entry_details(entry: Dict[str, Any]):
    ...
    if 'extra_info' in entry:
        ...
        print(f"  Problem: Level {extra.get('level', '?')}, "
...
def main():
    ...
    if args.files:
        ...
        all_files = sorted(glob.glob(os.path.join(rollout_dir, "rollout_data_*.json")),
